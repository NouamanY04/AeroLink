    ...
    <form action="{{ route('flights.update', $flight) }}" method="POST">
        ...
        <div class="mb-4">
            ...
            <select name="airline_id" class="mt-1 block w-full border-gray-300 rounded shadow-sm" required>
                ...
                <option value="{{ $airline->id }}"
                    {{ old('airline_id', $flight->airline_id) == $airline->id ? 'selected' : '' }}>{{ $airline->name }}
                </option>
                @endforeach
            </select>
        ...
        <div class="mb-4">
            ...
            <input type="datetime-local" name="departure_time"
                ...
                value="{{ old('departure_time', date('Y-m-d\TH:i', strtotime($flight->departure_time))) }}">
        ...
        <div class="mb-4">
            ...
            <input type="datetime-local" name="arrival_time" class="mt-1 block w-full border-gray-300 rounded shadow-sm"
                required value="{{ old('arrival_time', date('Y-m-d\TH:i', strtotime($flight->arrival_time))) }}">
        ...
        <div class="mb-4">
            ...
            <select name="status" class="mt-1 block w-full border-gray-300 rounded shadow-sm" required>
                <option value="scheduled" {{ old('status', $flight->status) == 'scheduled' ? 'selected' : '' }}>
                    Scheduled</option>
                <option value="delayed" {{ old('status', $flight->status) == 'delayed' ? 'selected' : '' }}>Delayed
                </option>
                <option value="cancelled" {{ old('status', $flight->status) == 'cancelled' ? 'selected' : '' }}>
                    Cancelled</option>
                <option value="completed" {{ old('status', $flight->status) == 'completed' ? 'selected' : '' }}>
                    Completed</option>
            </select>
        ...
        <div class="mb-4">
            ...
            <select name="class" class="mt-1 block w-full border-gray-300 rounded shadow-sm" required>
                ...
                <option value="business" {{ old('class', $flight->class) == 'business' ? 'selected' : '' }}>Business
                </option>
                <option value="first" {{ old('class', $flight->class) == 'first' ? 'selected' : '' }}>First Class
                </option>
            </select>
        ...
        <div class="mb-4">
            ...
            <select name="type_vol" class="mt-1 block w-full border-gray-300 rounded shadow-sm" required>
                ...
                <option value="Round-trip" {{ old('type_vol', $flight->type_vol) == 'Round-trip' ? 'selected' : '' }}>
                    Round-trip</option>
                <option value="One-way" {{ old('type_vol', $flight->type_vol) == 'One-way' ? 'selected' : '' }}>One-way
                </option>
            </select>
        ...
        <div class="mb-4">
            ...
            <input type="number" step="0.01" name="price" class="mt-1 block w-full border-gray-300 rounded shadow-sm"
                required value="{{ old('price', $flight->price) }}">
